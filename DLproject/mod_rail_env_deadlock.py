...
class RailEnv(Environment):
    ...
    def __init__(self,
        ...
        # Deadlock List
        self.deadlocks_current = [0] * number_of_agents
        self.deadlocks_coming = [0] * number_of_agents
    ...
    def _step_agent(self, i_agent, action: Optional[RailEnvActions] = None):
        ...
        if np.isclose(agent.speed_data['position_fraction'], 0.0, rtol=1e-03):
            ...
            if action is None:
                ...
                # Agent might be in deadlock
                print('Do_nothing \n')
                self.rewards_dict[i_agent] += self.temp_penalty
        ...
        if agent.moving:
            ...
            if agent.speed_data['position_fraction'] > 1.0 or np.isclose(agent.speed_data['position_fraction'], 1.0,
                ...
                if cell_free:
                    ...
                    agent.deadlocks_current = 0
                    agent.deadlocks_coming = 0
                else: # Deadlock Punishment - Check if Deadlock or Malfunction, Reward Deadlock with -10
                    self.deadlocks_coming[i_agent] = +1
                    for iter in range(self.get_num_agents()):
                        if self.agents[iter].position == new_position:
                            malfunction = self.agents[iter].malfunction_data['malfunction'] > 0
                    
                    if (self.deadlocks_coming[i_agent] == 2) and (not malfunction):
                            self.rewards_dict[i_agent] += self.lock_penalty
                            self.deadlocks_current[i_agent] = 1
                            self.counter_potential += 1
    ...
    def _avoid_deadlock(self, agent: EnvAgent, new_position: IntVector2D, action: RailEnvActions):
        """
        Avoid Deadlock
        """
